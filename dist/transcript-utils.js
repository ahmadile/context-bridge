...
exports.LOCAL_TRANSCRIPT_MAX_CHARS = void 0;
exports.prepareTranscriptForLocal = prepareTranscriptForLocal;
exports.findTranscriptExcerpt = findTranscriptExcerpt;
exports.segmentTranscript = segmentTranscript;
exports.suggestBestSegmentIndex = suggestBestSegmentIndex;
exports.buildTranscriptFromSegment = buildTranscriptFromSegment;
exports.compactHistoryForLocal = compactHistoryForLocal;
/** Taille max recommandée pour le prompt système local (caractères). */
exports.LOCAL_TRANSCRIPT_MAX_CHARS = 6000;
/**
 * Réduit une transcription pour l'IA locale : début + fin + note explicative.
 */
function prepareTranscriptForLocal(transcript, maxChars = exports.LOCAL_TRANSCRIPT_MAX_CHARS) {
 ...
 */
function findTranscriptExcerpt(transcript, query, windowChars = 2500) {
...
const ACTION_KEYWORDS = /\b(install|npm|yarn|pnpm|create|fichier|file|code|étape|step|commande|run|build|import|export|function|class|composant|component|erreur|fix|corriger|ajouter|modifier|mkdir|terminal|cursor|vscode)\b/gi;
/**
 * Découpe une transcription en segments (~taille cible) pour choisir par où commencer.
 */
function segmentTranscript(transcript, targetChunkChars = 4500) {
    const trimmed = transcript.trim();
    if (!trimmed)
        return [];
    const timestampSplitRe = /(?=(?:\n|^)(?:\[\d{1,2}:\d{2}(?::\d{2})?\]|\d{1,2}:\d{2}(?::\d{2})?)[\s:])/m;
    const timestampBlocks = trimmed.split(timestampSplitRe);
    let rawParts = timestampBlocks.length > 1
 ...
 */
function suggestBestSegmentIndex(segments) {
    if (segments.length === 0)
        return 0;
    let best = 0;
 ...
 */
function buildTranscriptFromSegment(segments, startIndex, maxChars = exports.LOCAL_TRANSCRIPT_MAX_CHARS) {
    if (segments.length === 0)
        return '';
...
function compactHistoryForLocal(history, maxMessages = 8, maxCharsPerMessage = 600) {

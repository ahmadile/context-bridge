...
export interface CodeSignature {
    type: 'function' | 'class' | 'interface' | 'type' | 'export' | 'import' | 'variable';
    name: string;
    signature: string;
    line: number;
}
 ...
 */
export function extractSignatures(content: string, filePath: string): CodeSignature[] {
...
function extractJSSignatures(content: string): CodeSignature[] {
...
function extractPythonSignatures(content: string): CodeSignature[] {
    const signatures: CodeSignature[] = [];
    const lines = content.split('\n');
...
function extractJavaSignatures(content: string): CodeSignature[] {
    const signatures: CodeSignature[] = [];
    const lines = content.split('\n');
 ...
 */
export function formatSignatures(signatures: CodeSignature[], filePath: string): string {
    if (signatures.length === 0) return `// ${filePath}: No signatures extracted`;
    ...
    if (imports.length > 0) {
        result += `// --- Imports ---\n`;
        for (const s of imports) result += `${s.signature}\n`;
        result += '\n';
    }
    if (types.length > 0) {
        result += `// --- Types & Interfaces ---\n`;
        for (const s of types) result += `${s.signature}  // line ${s.line}\n`;
        result += '\n';
    }
    if (classes.length > 0) {
        result += `// --- Classes ---\n`;
        for (const s of classes) result += `${s.signature}  // line ${s.line}\n`;
        result += '\n';
    }
    if (functions.length > 0) {
        result += `// --- Functions ---\n`;
        for (const s of functions) result += `${s.signature}  // line ${s.line}\n`;
        result += '\n';
    }
    if (variables.length > 0) {
        result += `// --- Exports ---\n`;
        for (const s of variables) result += `${s.signature}  // line ${s.line}\n`;
        result += '\n';
    }
    if (exports.length > 0) {
        for (const s of exports) result += `${s.signature}  // line ${s.line}\n`;
    }
